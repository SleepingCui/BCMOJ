...
def exams():
    query = request.args.get('q', '').strip()
    page = request.args.get('page', 1, type=int)
    context = get_exam_list(query, page)
...
def submit_exam(exam_id):
    ...
    user_result, error = handle_exam_submission(exam_id, user_id, request.form)

    if error:
        return error, 404

    return redirect(url_for('exam_result', userid=user_id, resultid=user_result.result_id))

@app.route('/exam/teacher')
def exam_manage():
    usergroup = session.get('usergroup')
    if usergroup not in ['admin', 'teacher']:
        abort(403)
    return render_template('teacher_exam_manage.html')

@app.route('/exam/teacher/api')
@teacher_required
def api_get_teacher_exam_data():
    return get_teacher_exam_data()

@app.route('/exam/teacher/api/create_exam', methods=['POST'])
@teacher_required
def api_create_exam():
    data = request.json
    return create_exam(data)

@app.route('/exam/teacher/api/update_exam', methods=['POST'])
@teacher_required
def api_update_exam():
    data = request.json
    return update_exam(data)

@app.route('/exam/teacher/api/delete_exam', methods=['POST'])
@teacher_required
def api_delete_exam():
    exam_id = request.json.get("exam_id")
    return delete_exam(exam_id)

@app.route('/exam/result/<int:userid>/<int:resultid>')
def exam_result(userid, resultid):
    data = get_exam_result_data(userid, resultid)
    return render_template('exam_result.html', **data)

@app.route('/exam/results')
@app.route('/exam/results/page/<int:page>')
@login_required
def exam_results(page=1):
    userid = session.get('user_id')
    results, total_pages = get_results_list(userid, page)
    return render_template('result_list.html', results=results, userid=userid, page=page, total_pages=total_pages)

@app.route('/exam/admin_results', methods=['GET'])
@app.route('/exam/admin_results/page/<int:page>', methods=['GET'])
@login_required
def admin_exam_results(page=1):
    search = request.args.get('search', '', type=str)
    results, total_pages = get_admin_results(page, search)

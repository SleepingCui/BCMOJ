async function fetchTeacherData() {
        ...
        function renderProblemEditor(p) {
            ...
            div.innerHTML = `
                <hr>
                <input value="${p.title}" placeholder="标题" style="width: 100%;"><br><br>
                <label>题目描述：</label><br>
                <textarea style="width: 100%; height: 150px;">${p.description}</textarea><br><br>
                时间限制(ms): <input type="number" value="${p.time_limit}"><br><br>
                <div class="examples">
                    <label>样例：</label>
                    ${p.examples.map(e => `
        ...
        function saveProblem(btn, problem_id = null) {
            ...
            if (problem_id) {
                ...
                axios.post('/teacher/api/update_problem', data).then(() => {
                    alert('题目更新成功！')
                    fetchTeacherData()
                    document.getElementById('problemEditor').innerHTML = ''
                })
            } else {
                axios.post('/teacher/api/create_problem', data).then(() => {
                    alert('题目创建成功！')
                    fetchTeacherData()
                    document.getElementById('problemEditor').innerHTML = ''
                })
        ...
        function deleteProblem(problem_id) {
            if (confirm('确定要删除这个题目吗？')) {
                axios.post('/teacher/api/delete_problem', { problem_id }).then(() => {
                    alert('题目删除成功！')
                    fetchTeacherData()
                    document.getElementById('problemEditor').innerHTML = ''
                })

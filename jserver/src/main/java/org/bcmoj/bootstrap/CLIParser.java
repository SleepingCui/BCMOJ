...
public class CLIParser {
    ...
    public static CommandLine parse(String[] args) {
        ...
        options.addOption(Option.builder().longOpt("host").hasArg().argName("IP").required().desc("Server IP address to bind to (e.g., 0.0.0.0 for all interfaces)").build());
        options.addOption(Option.builder().longOpt("port").hasArg().argName("PORT").required().desc("Port number for the server to listen on (e.g., 12345)").build());
        options.addOption(Option.builder().longOpt("kwfile").hasArg().argName("FILE").required().desc("Path to the keyword file containing reserved words for the judge system").build());
        ...
        try {
            ...
            new HelpFormatter().printHelp("java -jar BCMOJ.jar", options, true);

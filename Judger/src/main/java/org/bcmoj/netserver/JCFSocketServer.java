...
public class JCFSocketServer {
    ...
    public void start(int port) {
        try {
            logger.info("Attempting to bind server socket to port: {}", port);
    ...
    private static class ClientHandler implements Runnable {
        ...
        public void run() {
            ...
            clientLogger.info("[{}] Client handler started", clientAddress);
            try (DataInputStream dis = new DataInputStream(clientSocket.getInputStream());
                 DataOutputStream dos = new DataOutputStream(clientSocket.getOutputStream())) {

                clientLogger.info("[{}] Streams initialized for communication", clientAddress);
                ...
                if (!validator.validate(jsonConfig, dos)) {
                    clientLogger.warn("[{}] JSON validation failed. Aborting client processing.", clientAddress);
                    return;
                }
                clientLogger.info("[{}] JSON validation passed", clientAddress);

                clientLogger.info("[{}] Preparing to invoke JudgeServer...", clientAddress);
                File KWFilePath = new File(ConfigProcess.GetConfig("KeywordsFilePath"));
                if (!KWFilePath.exists()) {
                    clientLogger.warn("[{}] Keywords file not found at path: {}", clientAddress, KWFilePath.getAbsolutePath());
                } else {
                    clientLogger.debug("[{}] Keywords file found: {}", clientAddress, KWFilePath.getAbsolutePath());
                }

                String jserverResponse = JudgeServer.JServer(jsonConfig, outputFile, KWFilePath);
                clientLogger.info("[{}] JudgeServer execution complete. Response: {}", clientAddress, jserverResponse);
                ...
                if (outputFile != null && outputFile.exists()) {
                    try {
                        if (outputFile.delete()) {
                            clientLogger.info("[{}] Deleted temporary file: {}", clientAddress, outputFile.getName());
                        } else {
                            clientLogger.warn("[{}] Failed to delete temporary file: {}", clientAddress, outputFile.getName());
                        }
